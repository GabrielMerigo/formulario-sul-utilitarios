...
export const fetchMainImageUrl = async (
  ...
  try {
    ...
    const response = await getDownloadURL(
      ref(storage, `${vehicleId}/mainImage`)
    );
...
export const fetchImageUrl = async (
  ...
  try {
    const response = await getDownloadURL(
      ref(storage, `${vehicleId}/${cloudImage.name}`)
    );
    setState(response);
  } catch ({ name, message }) {
    toast(
      "Houve um erro ao carregar a(s) imagem(s) do(s) veiculo(s):\n" +
        `${message}:${name}`,
      {
        className: "error",
      }
...
export const setManyImagesUrls = (
  ...
  try {
    ...
    cloudImageArray.forEach(async (image) => {
      await getDownloadURL(ref(storage, `${vehicleId}/${image.name}`)).then(
        (url: string) => {
...
export const deleteImageByStorageReference = async (
  ...
  try {
    const deleteFile = deleteObject(
      ref(storage, `${vehicleId}/${cloudImageName}`)
    );
    const remainingImages = cloudImages.filter(
      (image) => image.name !== cloudImageName
    );
...
export const deleteImageByURL = async (
  ...
  try {
    const deleteFile = await deleteObject(
      ref(storage, `${vehicleId}/${imageName}`)
    );
...
export const deleteVehicleImagesFolder = async (vehicleId: string) => {
  try {
    ...
    imagesList.items.forEach((image) => {
      const deleteFile = deleteObject(
        ref(storage, `${vehicleId}/${image.name}`)
      );
    });
  } catch ({ message, name }) {
    toast(
...
export const uploadMainImage = async (
  ...
  try {
    ...
    toast(
      "Houve um erro com ao registrar a imagem principal:\n" +
        `${message}:${name}`,
      {
        className: "error",
      }
...
export const uploadImages = async (vehicleId: string, images: ImageFile[]) => {
  try {
    ...
    toast(
      "Houve um erro com ao registrar as imagens:\n" + `${message}:${name}`,
      {
        className: "error",
      }
    );
